...
"""
    ...
    def test_reload_topology_change(self):
        ...
        for new_dp in new_dps:
            ...
            (deleted_ports, changed_ports, added_ports, changed_acl_ports,
             ...
             _, deleted_meters, added_meters,
             changed_meters) = valve.dp.get_config_changes(valve.logger, new_dp)
            for port in valve.dp.stack_ports:
                if not all_ports_changed:
                    self.assertIn(port.number, changed_ports,
    ...
    def test_reload_vlan_change(self):
        ...
        self.update_and_revert_config(self.CONFIG, self.NEW_VLAN_CONFIG, 'warm')
        import os
        if os.path.exists(self.config_file):
            with open(self.config_file) as config_file:
                existing_config = config_file.read()
        ...
        for new_dp in new_dps:
            ...
            (deleted_ports, changed_ports, added_ports, changed_acl_ports,
             ...
             changed_meters) = valve.dp.get_config_changes(valve.logger, new_dp)
            for port in valve.dp.stack_ports:
                self.assertNotIn(port.number, changed_ports,
